...
from .utils import get_base_name, make_dataset, read_json_file, write_json_file, anno_exists, create_base_annotation
...
class IAPDataset():
    ...
    @staticmethod
    def create_dataset(dataset_root, parser=None, anno_file='auto_annotations.json', img_dir='images', meta_file='metadata.yml', continue_flag=False, from_empty=False, **kwargs):
        ...
        if '/' in img_dir or '/' in meta_file or '/' in anno_file:
            raise ValueError(f"{img_dir}, {meta_file}, {anno_file}: leaf name is required")
        ...
        if parser:
            logger.info(f"create dataset with auto-parsing annotation into {anno_path}.")
            parser.parse(img_path, out_json_file=anno_path, continue_flag=continue_flag)
            anno_dict = parser.out_json
        else:
            if os.path.exists(anno_path):
                logger.warning(f"create dataset with annotation {anno_path} while parser version is unknown, you should set it manually.")
                anno_dict = read_json_file(anno_path)
            else:
                logger.info(f"create dataset with base name annotation into {anno_path}.")
        ...
        metadata['parser_version'] = kwargs.get('parser_version', parser.__version__ if parser else None)
        metadata['dataset_version'] = kwargs.get('dataset_version', IAPDataset.__version__)
        metadata = CN(metadata)
        with open(metafile_path, mode='w+') as f:
            metadata.dump(stream=f, sort_keys=False)

    @classmethod
    def merge_dataset(cls, dataset_root, sub_datasets, anno_file='auto_annotations.json', img_dir='images', meta_file='metadata.yml'):
        ...
        assert all([iap.metadata['from_processors'] == sub_datasets[0].metadata['from_processors'] for iap in sub_datasets])
        assert all([iap.metadata['parser_version'] == sub_datasets[0].metadata['parser_version'] for iap in sub_datasets])
        ...
        _ = [shutil.move(p, img_path) for dataset in sub_datasets for p in dataset.img_files]
        anno_dict = {}
        ...
        # 3- set meta message
        meta_message = {}
        meta_message['from_dataset'] = [dataset.metadata['from_dataset']+[dataset.metadata['name']] for dataset in sub_datasets]
        ...
        cls.create_dataset(dataset_root, anno_file=anno_file, img_dir=img_dir, meta_file=meta_file, **meta_message)

    def _check_datasize(self):
        if self.metadata['size'] != len(self.anno_dict):
            logger.info('Annotation files size (%d) is not equal to dataset size (%d)' % (len(self.anno_dict), self.metadata['size']))
        if self.metadata['size'] != len(self.img_files):
            logger.info('Image files size (%d) is not equal to dataset size (%d)' % (len(self.img_files), self.metadata['size']))
    ...
    def _set_dataset(self):
        ...
        assert(parser_v.domain == 'lamply')   # parser and processor heavily rely on anno's structure
        ...
        if dataset_v < parse_version('lamply-1.2'):
            ...
            self.metadata['from_dataset'] = [] if self.metadata['from_dataset'] is None else [self.metadata['from_dataset']]
            self.metadata['from_processors'] = [] if self.metadata['from_processors'] is None else self.metadata['from_processors']
    ...
    def load_data(self, idx=0, name=None, rgb_out=False, anno_only=False):
        ...
        if anno_only:
            ...
            if rgb_out:
                self.current_data = np.array(Image.open(self.img_files[idx])), self.anno_dict[self.current_name]
            else:
                # IMPROVE: cv2.IMREAD_COLOR is not a good idea, because the alpha channel will be missing
                self.current_data = cv2.imread(self.img_files[idx], cv2.IMREAD_COLOR), self.anno_dict[self.current_name]
    ...
    def update_state(self, dump=True):
        ...
        self.name_list = [get_base_name(p) for p in self.img_files]
        anno_dict_ordered = OrderedDict(sorted(self.anno_dict.items(), key=lambda kv: kv[0]))
        self._check_datasize()
        if dump:
    ...
    def delete_data(self, idx=0, name=None):
        ...
        if anno_exists(self.anno_dict[name], 'masks'):
            [os.remove(mask_path) for mask_path in self.anno_dict[name]['masks'].values()]
            ...
            try:
                for processor in processors:
                    if isinstance(processed_data, list):
                        processed_data = [processor(p_data) for p_data in processed_data]
    ...
    def pipeline(self, dataset_root, processors, idx_list=[], anno_file='auto_annotations.json', img_dir='images', meta_file='metadata.yml'):
        ...
        new_config = {}
        new_config['from_dataset'] = self.metadata['from_dataset'] + [self.metadata['name']]
        new_config['from_processors'] = [processor.dump_config() for processor in processors]
        new_config['parser_version'] = self.metadata['parser_version']
        new_config['dataset_version'] = self.__version__
        self.create_dataset(dataset_root, anno_file=anno_file, img_dir=img_dir, meta_file=meta_file, from_empty=True, **new_config)
        ...
        for i in tqdm(process_id):
            ...
            try:
                for processor in processors:
                    if isinstance(processed_data, list):
                        processed_data = [processor(p_data) for p_data in processed_data if isinstance(p_data, tuple)]
                    elif isinstance(processed_data, tuple):
                        processed_data = processor(processed_data)
                    else:
                        break
                ...
                processed_data = None
            if isinstance(processed_data, list):
                [nextgen.add_data(p_data, state_update=False) for p_data in processed_data if isinstance(p_data, tuple)]
            elif isinstance(processed_data, tuple):
    ...
    def display_data(self, display_func, idx=None, name=None, disable_domain=[]):
        ...
        if not isinstance(disable_domain, list):
            raise RuntimeError('disable_domain should be a list with domain: [\'landmarks\', \'genderage\', \'headpose\']')
        if idx:
            self.load_data(idx)
        elif name:
