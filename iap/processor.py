...
class func_processor:
    ...
    def dump_config(self):
        config = {
            "__name__": "func_processor",
            "source": inspect.getsource(self.process_func),
        }
...
class select_processor:
    ...
    def dump_config(self):
        config = {
            "__name__": "select_processor",
            "cleaner": self.cleaner.__class__.__name__,
        }
        config.update(self.params)
        return config
...
class if_processor:
    ...
    def dump_config(self):
        ...
        config.update(
            {
                "lambda": [
                    inspect.getsource(if_expression) for if_expression in self.params
                ]
            }
        )
        return config
