...
def check_image_format(img, allow_float=False, fix_channel=True):
    ...
    if fix_channel:
        if len(img.shape) == 2 or len(img.shape) == 3:
            ...
            raise ValueError(
                f"ERROR: image is not regular image! img.shape: {img.shape}"
            )
    if img.dtype != np.uint8 and not allow_float:
        print(
            f"Warning: image type is not uint8 and {img.dtype} is not allow, auto-sacling into np.uint8..."
        )
        img = img * 1.0
...
def write_json_file(
    file, obj, override=False, use_msgpack=False, min_size=10, **dump_kwargs
):
...
def create_base_annotation(img_path):
    ...
    [
        anno_dict.update({get_base_name(img_f): set_image_anno(get_base_name(img_f))})
